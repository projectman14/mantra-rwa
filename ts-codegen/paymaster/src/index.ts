...
import { PaymasterClient , PaymasterQueryClient } from './codegen/Paymaster.client';
...
export async function getCosmWasmClient(
  ...
  export async function GetPayment(contractAddressdynamic:string) {
    // const address = connect()
    const client = await getCosmWasmClient("https://rpc.hongbai.mantrachain.io" , mnemonic , "mantra");
    const dinonum = new PaymasterQueryClient(client , contractAddressdynamic);

    return dinonum;
  }

  export async function SetAutopay(contractAddressdynamic:string) {
