...
import { LoanDatabaseClient , LoanDatabaseQueryClient } from './codegen/LoanDatabase.client';
...
export async function getCosmWasmClient(
  ...
  export async function GiveLoan() {
    ...
    const contractAddress = "mantra19u9j7yhj4ueqmnnw8xzw6ppxs4egugzuvljqqtx5ksrzfcaqp3uqxnrqpz";
    const dinonum = new LoanDatabaseClient(client , address , contractAddress); 

    return dinonum;
  }

  export async function GetLoanContratAddress() {
    const address = connect()
    const client = await getCosmWasmClient("https://rpc.hongbai.mantrachain.io" , mnemonic , "mantra");
    const contractAddress = "mantra19u9j7yhj4ueqmnnw8xzw6ppxs4egugzuvljqqtx5ksrzfcaqp3uqxnrqpz";
    const dinonum = new LoanDatabaseQueryClient(client , contractAddress);
